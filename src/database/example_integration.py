...
class WeatherDashboardIntegration:
    ...
    async def get_location_weather_history(self, location: str, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            # Get history
            history = await self._data_service.get_weather_history(
                location=location,
                days=days
            )
    ...
    async def get_search_history(self, limit: int = 10) -> list[str]:
        ...
        """
        return await self._data_service.get_recent_searches(
            user_id=self._current_user,
            limit=limit
        )
    ...
    # Journal integration
    async def create_mood_entry(self, mood_score: float, notes: str = "", weather_data: Dict = None) -> bool:
    ...
    async def get_mood_journal(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get trends
            trends = await self._data_service.get_mood_trends(
                user_id=self._current_user,
                days=days
            )
    ...
    async def import_user_data(self, import_path: Path, validate_only: bool = False) -> Dict[str, Any]:
        """Import user data.
