    ...
    async def get_current_version(self) -> Optional[str]:
        ...
        try:
            async with self._db_manager.get_async_session() as session:
                ...
                result = await session.execute(text(
                    "SELECT version FROM database_migrations ORDER BY applied_at DESC LIMIT 1"
                ))
    ...
    async def rollback_migration(self, version: str) -> bool:
        ...
        try:
            async with self._db_manager.get_async_session() as session:
                ...
                await session.execute(text(
                    "DELETE FROM database_migrations WHERE version = :version"
                ), {'version': version})
        ...
        try:
            ...
            for applied in applied_migrations:
                if applied.version in migration_map:
                    ...
                    validation_results[applied.version] = (
