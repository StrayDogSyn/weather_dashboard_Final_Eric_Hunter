...
class UIComponentsMixin:
    ...
    def center_window(self):
        ...
        try:
            ...
            if screen_width is None or screen_height is None:
                self.logger.error(
                    f"Screen dimensions are None: width={screen_width}, height={screen_height}"
                )
                # Fallback geometry
            ...
            if screen_width <= 0 or screen_height <= 0:
                self.logger.error(
                    f"Invalid screen dimensions: width={screen_width}, height={screen_height}"
                )
                # Fallback geometry
                self.geometry("1800x1200")
                return
            ...
            center_y = max(
                40, min(center_y, screen_height - window_height - 40)
            )  # 40px top margin for taskbar

            # Set window geometry with center position
            self.geometry(f"{window_width}x{window_height}+{center_x}+{center_y}")

            self.logger.info(
                f"Window centered at {center_x},{center_y} with size {window_width}x{window_height} on screen {screen_width}x{screen_height}"
            )
    ...
    def update_component_scaling(self, width, height):
        ...
        try:
            ...
            self.logger.debug(
                f"Component scaling: width={width} ({type(width)}), height={height} ({type(height)})"
            )
            ...
            self.logger.debug(
                f"Calculated font sizes: base={base_font_size}, large={large_font_size}"
            )
            ...
            if hasattr(self, "temperature_chart") and hasattr(
                self.temperature_chart, "update_size"
            ):
    ...
    def _create_header(self) -> None:
        ...
        self.header_frame = ctk.CTkFrame(
            self, fg_color=self.CARD_COLOR, corner_radius=0, height=100, border_width=0
        )

        # Header accent strip
        self.header_accent = ctk.CTkFrame(
            self.header_frame, fg_color=self.ACCENT_COLOR, height=2, corner_radius=0
        )
    ...
    def _update_all_components_for_location(self, location_data: dict):
        ...
        try:
            self.logger.info(
                f"Updating all components for location: {location_data.get('name', 'Unknown')}"
            )
    ...
    def _update_location_labels(self, location_data: dict):
        ...
        try:
            display_name = location_data.get(
                "display", location_data.get("name", "Unknown Location")
            )
            ...
            if hasattr(self, "enhanced_weather_display") and hasattr(
                self.enhanced_weather_display, "location_label"
            ):
    ...
    def _update_maps_component(self, location_data: dict):
        ...
        try:
            if hasattr(self, "maps_component"):
                coordinates = {"lat": location_data.get("lat"), "lon": location_data.get("lon")}
    ...
    def _update_location_settings(self, location_data: dict):
        ...
        try:
            ...
            if country and hasattr(self, "config_service"):
                units = (
                    "imperial" if country.upper() in ["US", "USA", "UNITED STATES"] else "metric"
                )
    ...
    def _create_status_bar(self) -> None:
        ...
        self.status_frame = ctk.CTkFrame(
            self, fg_color=self.CARD_COLOR, corner_radius=0, height=30, border_width=0
        )

        # Status accent strip
        self.status_accent = ctk.CTkFrame(
            self.status_frame, fg_color=self.ACCENT_COLOR, height=1, corner_radius=0
        )
    ...
    def _search_weather(self, city_name: str = None):
        ...
        try:
            ...
            self._show_status_message(
                f"Weather search for '{city_name}' - Service integration needed", "info"
            )
    ...
    def _show_status_message(self, message: str, message_type: str = "info"):
        ...
        try:
            ...
            if hasattr(self, "logger"):
                getattr(self.logger, message_type if message_type != "warning" else "warning")(
                    message
                )
    ...
    def _setup_search_entry(self, parent):
        ...
        try:
            ...
            self.search_entry = ctk.CTkEntry(
                parent, placeholder_text="Enter city name...", width=300, height=35
            )
            ...
            if hasattr(self, "connection_status"):
                if connected:
                    self.connection_status.configure(
                        text="🌐 Connected", text_color=self.ACCENT_COLOR
                    )
                else:
