...
class DashboardController(BaseDashboard):
    ...
    def _configure_tab_grids(self):
        ...
        tabs = [
            self.weather_tab, self.comparison_tab, self.ml_comparison_tab,
            self.activities_tab, self.maps_tab, self.settings_tab
        ]
    ...
    def _initialize_tab_managers(self):
        ...
        self.weather_tab_manager = WeatherTabManager(
            self, self.weather_tab
        )
    ...
    def _on_location_selected(self, location_result):
        ...
        if self.header_component:
            self.header_component.update_current_location(location_result.get('display_name', 'Unknown'))
        
        # Update status
        if self.status_bar_component:
            self.status_bar_component.show_loading(f"Loading weather for {location_result.get('display_name', 'location')}...")
        
        # Trigger weather update (this would typically call the weather service)
        # For now, just update the status
        if self.status_bar_component:
            self.status_bar_component.safe_after(2000, lambda: self.status_bar_component.clear_status())
    ...
    def update_weather_data(self, weather_data):
        ...
        if self.status_bar_component:
            ...
            self.status_bar_component.safe_after(3000, lambda: self.status_bar_component.clear_status())
