...
# Services
from ...services.ml_weather_service import MLWeatherService, WeatherProfile, SimilarityResult, RecommendationResult
...
class PreferenceDialog(ctk.CTkToplevel):
    ...
    def _setup_ui(self):
        ...
        title_label = ctk.CTkLabel(
            main_frame,
            text="🌤️ Tell us your ideal weather!",
    ...
    def _create_preference_controls(self, parent):
        ...
        ctk.CTkLabel(temp_frame, text="🌡️ Ideal Temperature (°C):", font=("JetBrains Mono", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(humidity_frame, text="💧 Ideal Humidity (%):", font=("JetBrains Mono", 12, "bold")).pack(anchor="w")
        ...
        self.humidity_slider.configure(command=lambda v: self.humidity_value.configure(text=f"{v:.1f}%"))
        
        # Wind speed preference
        wind_frame = ctk.CTkFrame(parent, fg_color="transparent")
        wind_frame.pack(fill="x", pady=10)
        
        ctk.CTkLabel(wind_frame, text="💨 Ideal Wind Speed (m/s):", font=("JetBrains Mono", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(pressure_frame, text="🌀 Ideal Pressure (hPa):", font=("JetBrains Mono", 12, "bold")).pack(anchor="w")
        ...
        self.pressure_slider.configure(command=lambda v: self.pressure_value.configure(text=f"{v:.0f} hPa"))
        
        # UV Index preference
        uv_frame = ctk.CTkFrame(parent, fg_color="transparent")
        uv_frame.pack(fill="x", pady=10)
        
        ctk.CTkLabel(uv_frame, text="☀️ Max UV Index:", font=("JetBrains Mono", 12, "bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(type_frame, text="🌈 Preferred Weather Type:", font=("JetBrains Mono", 12, "bold")).pack(anchor="w")
        self.weather_type = ctk.CTkOptionMenu(
            type_frame,
            values=["☁️ Cool & Cloudy", "🔥 Warm & Dry", "🌧️ Mild & Humid", "❄️ Cool & Crisp", "🌴 Tropical"]
        )
...
class MLComparisonPanel(ctk.CTkFrame):
    ...
    def _create_header(self):
        ...
        title_label = ctk.CTkLabel(
            header_frame,
            text="🧠 AI-Powered Weather Analysis",
    ...
    def _create_control_panel(self):
        ...
        clear_btn = ctk.CTkButton(
            city_input_frame,
            text="🗑️ Clear All",
            command=self._clear_cities,
            width=100
        )
        clear_btn.pack(side="left")
        
        # Selected cities display
        self.cities_display = ctk.CTkTextbox(
            city_section,
            height=60,
        ...
        ctk.CTkButton(
            row2,
            text="🧠 Get Recommendations",
            command=self._show_preference_dialog,
            width=180
        ).pack(side="left", padx=(0, 10))
        ...
        ctk.CTkButton(
            ...
            command=self._show_detailed_analysis,
            width=150
        ).pack(side="left", padx=(0, 10))
    ...
    def _show_visualization(self, chart_type: str):
        ...
        if len(self.weather_profiles) < 2:
            messagebox.showwarning("Insufficient Data", "Please add at least 2 cities for analysis.")
            return
    ...
    def _generate_chart_insights(self, chart_type: str):
        ...
        try:
            ...
            if chart_type == "similarity":
                    ...
                    for i in range(len(self.weather_profiles)):
                        for j in range(i + 1, len(self.weather_profiles)):
                            ...
                            if similarity > max_similarity:
                                ...
                                most_similar_pair = (self.weather_profiles[i].city_name, self.weather_profiles[j].city_name)
                    
                    if most_similar_pair:
                        insights.append(f"🎯 Most Similar Cities: {most_similar_pair[0]} and {most_similar_pair[1]} ({max_similarity:.1%} similarity)")
                        ...
                        insights.append(f"📊 Key Factors: {', '.join(similarity_result.dominant_factors)}")
                ...
                insights.append(f"🔥 Hottest: {hottest_city.city_name} ({hottest_city.temperature:.1f}°C)")
                insights.append(f"❄️ Coolest: {coldest_city.city_name} ({coldest_city.temperature:.1f}°C)")
                insights.append(f"💧 Most Humid: {most_humid.city_name} ({most_humid.humidity:.0f}%)")
                insights.append(f"🏜️ Least Humid: {least_humid.city_name} ({least_humid.humidity:.0f}%)")
    ...
    def _show_preference_dialog(self):
        ...
        if len(self.weather_profiles) < 1:
            messagebox.showwarning("No Data", "Please add at least 1 city before getting recommendations.")
            return
    ...
    def _handle_preferences(self, preferences: Dict[str, Any]):
        ...
        try:
            recommendation = self.ml_service.recommend_city_by_preferences(preferences, self.weather_profiles)
            ...
            if recommendation:
                ...
                if 'differences' in comp_data:
                    ...
                    insights.append(f"   Temperature difference: {diffs.get('temperature', 0):.1f}°C")
                    insights.append(f"   Humidity difference: {diffs.get('humidity', 0):.1f}%")
                    insights.append(f"   Wind speed difference: {diffs.get('wind_speed', 0):.1f} m/s")
    ...
    def _show_detailed_analysis(self):
        ...
        if len(self.weather_profiles) < 2:
            messagebox.showwarning("Insufficient Data", "Please add at least 2 cities for detailed analysis.")
            return
    ...
    def _generate_detailed_analysis(self) -> str:
        ...
        if clusters:
            ...
            for cluster in clusters:
                ...
                if cluster.characteristics:
                    analysis.append(f"   Avg Temperature: {cluster.characteristics.get('avg_temperature', 0):.1f}°C")
                    analysis.append(f"   Avg Humidity: {cluster.characteristics.get('avg_humidity', 0):.0f}%")
                analysis.append("")
