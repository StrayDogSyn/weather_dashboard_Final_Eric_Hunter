...
class SimpleTemperatureChart(ctk.CTkFrame):
    ...
    def update_data(self, temperatures: List[float], timestamps: Optional[List[datetime]] = None) -> None:
        ...
        if timestamps:
            ...
            now = datetime.now()
            self.timestamps = [now - timedelta(hours=i) for i in range(len(self.temperatures)-1, -1, -1)]
        
        # Start animation
        self._start_animation()
        
    def set_data(self, temperatures: List[float], timestamps: Optional[List[datetime]] = None) -> None:
    ...
    def _draw_chart(self, animation_progress: float = 1.0) -> None:
        ...
        # Apply zoom and pan
        effective_width = chart_width * self.zoom_factor
        effective_height = chart_height * self.zoom_factor
        ...
        self._draw_temperature_bands(margin_left, margin_top, chart_width, chart_height, min_temp, max_temp)
        
        # Draw grid lines
        self._draw_grid_lines(margin_left, margin_top, chart_width, chart_height, min_temp, max_temp)
        
        # Calculate points with smooth interpolation
        points = self._calculate_smooth_points(current_temps, margin_left, margin_top, chart_width, chart_height, min_temp, temp_range)
        ...
        self._draw_min_max_markers(current_temps, points, margin_left, margin_top, chart_width, chart_height, min_temp, temp_range)
    ...
    def _draw_temperature_bands(self, x: int, y: int, width: int, height: int, min_temp: float, max_temp: float) -> None:
        ...
        if cold_y < y + height:
            self.canvas.create_rectangle(
                x, max(cold_y, y), x + width, y + height,
                fill=self.temp_colors["cold"], outline="", stipple="gray25"
            )
        
        if hot_y > y and cold_y > y:
            self.canvas.create_rectangle(
                x, max(hot_y, y), x + width, min(cold_y, y + height),
                fill=self.temp_colors["moderate"], outline="", stipple="gray25"
            )
        
        if hot_y > y:
            self.canvas.create_rectangle(
                x, y, x + width, min(hot_y, y + height),
                fill=self.temp_colors["hot"], outline="", stipple="gray25"
            )
    
    def _draw_grid_lines(self, x: int, y: int, width: int, height: int, min_temp: float, max_temp: float) -> None:
        ...
        for i in range(num_v_lines + 1):
            ...
            self.canvas.create_line(
                grid_x, y, grid_x, y + height,
                fill=self.grid_color, width=1, dash=(2, 2)
            )
    
    def _calculate_smooth_points(self, temps: List[float], x: int, y: int, width: int, height: int, min_temp: float, temp_range: float) -> List[Tuple[float, float]]:
    ...
    def _draw_temperature_points(self, points: List[Tuple[float, float]], temps: List[float]) -> None:
    ...
    def _draw_min_max_markers(self, temps: List[float], points: List[Tuple[float, float]], x: int, y: int, width: int, height: int, min_temp: float, temp_range: float) -> None:
        ...
        if min_idx * step < len(points):
            ...
            self.canvas.create_polygon(
                min_x, min_y - 15, min_x - 8, min_y - 25, min_x + 8, min_y - 25,
                fill=self.temp_colors["cold"], outline=self.text_color, width=1
            )
        ...
        if max_idx * step < len(points):
            ...
            # Max marker
            self.canvas.create_polygon(
                max_x, max_y + 15, max_x - 8, max_y + 25, max_x + 8, max_y + 25,
                fill=self.temp_colors["hot"], outline=self.text_color, width=1
            )
    ...
    def _draw_axes_labels(self, x: int, y: int, width: int, height: int, min_temp: float, max_temp: float) -> None:
