...
class ActivitySuggesterComponent(ctk.CTkFrame):
    ...
    def __init__(
        ...
        Args:
            ...
            gemini_service: Gemini AI service (legacy compatibility)
    ...
    def _create_header(self):
        ...
        header_frame = ctk.CTkFrame(
            self, height=80, fg_color=DataTerminalTheme.SURFACE_COLOR, corner_radius=15
        )
        ...
        ai_label = ctk.CTkLabel(title_frame, text="🤖", font=ctk.CTkFont(size=24))
    ...
    def _create_filters_sidebar(self, parent):
        ...
        filters_frame = ctk.CTkFrame(
            parent, width=280, fg_color=DataTerminalTheme.SURFACE_COLOR, corner_radius=15
        )
        ...
        filters_scroll = ctk.CTkScrollableFrame(filters_frame, fg_color="transparent")
    ...
    def _create_duration_filter(self, parent):
        ...
        self.duration_value_label = ctk.CTkLabel(
            duration_frame, text="120 minutes", text_color=DataTerminalTheme.TEXT_COLOR
        )
        self.duration_value_label.pack(anchor="w")

        # Update label when slider changes
        self.duration_slider.configure(
            command=lambda value: self.duration_value_label.configure(text=f"{int(value)} minutes")
        )
    ...
    def _create_difficulty_filter(self, parent):
        ...
        self.difficulty_value_label = ctk.CTkLabel(
            difficulty_frame, text="5 - Very Hard", text_color=DataTerminalTheme.TEXT_COLOR
        )
    ...
    def _create_suggestions_area(self, parent):
        ...
        suggestions_header = ctk.CTkFrame(
            suggestions_frame, height=60, fg_color=DataTerminalTheme.SURFACE_COLOR, corner_radius=15
        )
        ...
        self.suggestions_scroll = ctk.CTkScrollableFrame(suggestions_frame, fg_color="transparent")
        ...
        try:
            ...
            for i, raw_suggestion in enumerate(raw_suggestions):
                suggestion = ActivitySuggestion(
                    ...
                    title=raw_suggestion.get("title", "Activity"),
                    description=raw_suggestion.get("description", "No description available"),
                    category=self._map_category(raw_suggestion.get("category", "Outdoor")),
                    indoor=raw_suggestion.get("category", "Outdoor").lower() == "indoor",
                    duration_minutes=self._parse_duration(raw_suggestion.get("time", "1 hour")),
                    difficulty_level=2,  # Default difficulty
                    equipment_needed=self._parse_items(raw_suggestion.get("items", "")),
    ...
    def _map_category(self, category_str: str) -> ActivityCategory:
        ...
        category_mapping = {
            ...
            "educational": ActivityCategory.EDUCATIONAL,
    ...
    def _create_suggestion_card(self, suggestion: ActivitySuggestion, index: int) -> ctk.CTkFrame:
        ...
        location_indicator = ctk.CTkLabel(
            icon_frame, text="🏠" if suggestion.indoor else "🌤️", font=ctk.CTkFont(size=16)
        )
        ...
        # Difficulty stars
        difficulty_stars = "⭐" * suggestion.difficulty_level + "☆" * (
            5 - suggestion.difficulty_level
        )
        difficulty_label = ctk.CTkLabel(
            title_frame,
        ...
        suitability_color = (
            DataTerminalTheme.ACCENT_COLOR if suitability > 0.7 else DataTerminalTheme.WARNING_COLOR
        )
    ...
    def _show_no_suggestions_message(self):
        ...
        message_frame = ctk.CTkFrame(
            self.suggestions_scroll, fg_color=DataTerminalTheme.SURFACE_COLOR, corner_radius=15
        )
    ...
    def _show_error_message(self, message: str):
        ...
        error_frame = ctk.CTkFrame(
            self.suggestions_scroll, fg_color=DataTerminalTheme.ERROR_COLOR, corner_radius=15
        )
    ...
    def _apply_filters(self):
        ...
        self.selected_filters["cost_range"] = (
            self.cost_var.get().lower().replace(" only", "").replace(" cost", "")
        )
    ...
    def _filter_suggestions(
        ...
        for suggestion in suggestions:
            ...
            if (
                self.selected_filters["categories"]
                and suggestion.category not in self.selected_filters["categories"]
            ):
                continue
    ...
    def _plan_activity(self, suggestion: ActivitySuggestion):
        ...
        if plan:
            ...
            messagebox.showinfo(
                "Activity Planned", f"'{suggestion.title}' has been added to your activity plan!"
            )
