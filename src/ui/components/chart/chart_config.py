...
class ChartConfigMixin:
    ...
    def _configure_chart_appearance(self):
        ...
        self.ax.set_xlabel(
            "Time", color="#00ff88", fontsize=13, fontweight="bold", fontfamily="Segoe UI"
        )
        self.ax.set_ylabel(
            "Temperature (°C)",
            color="#00ff88",
    ...
    def apply_glassmorphic_styling(self):
        ...
        # Base layer
        base_bg = Rectangle(
            (0, 0), 1, 1, transform=self.ax.transAxes, facecolor="#0a0a0a", alpha=0.9, zorder=0
        )
        self.ax.add_patch(base_bg)

        # Glassmorphic overlay with gradient effect
        overlay_bg = Rectangle(
            (0, 0), 1, 1, transform=self.ax.transAxes, facecolor="#1a1a1a", alpha=0.3, zorder=1
        )
    ...
    def create_gradient_background(self, x_data, y_data):
        ...
        self.ax.fill_between(
            x_data, y_data, y_min, alpha=0.25, color="#00ff88", interpolate=True, zorder=2
        )

        # Secondary gradient for depth
        self.ax.fill_between(
            x_data, y_data, y_min, alpha=0.1, color="#ffffff", interpolate=True, zorder=1
        )
                        ...
                        spine.set_path_effects(
                            [
                                plt.matplotlib.patheffects.withStroke(
                                    linewidth=glow_width, foreground="#00ff88", alpha=glow_alpha
                                )
                            ]
                        )
        except Exception as e:
