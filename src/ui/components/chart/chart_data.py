...
class ChartDataMixin:
    ...
    def _generate_24h_data(self) -> Dict[str, List]:
        ...
        return {"dates": dates, "temperatures": temperatures, "humidity": humidity, "wind": wind}
        ...
        try:
            ...
            if "list" in forecast_data:
                ...
                for item in forecast_data["list"]:
                    ...
                    processed_data["visibility"].append(
                        item.get("visibility", 10000) / 1000
                    )  # Convert to km
    ...
    def smooth_data(self, data: List[float], window_size: int = 3) -> List[float]:
        ...
        for i in range(len(data)):
            ...
            if window_data:  # Check if window_data is not empty
                ...
                )  # Use original value or 0 as fallback

        return smoothed

    def calculate_temperature_trends(
        self, temperatures: List[float], dates: List[datetime]
    ) -> Dict[str, Any]:
            ...
            if interpolated_temps[i] is None:
                ...
                if prev_idx >= 0 and next_idx < len(interpolated_temps):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Interpolation division: {numerator} ({type(numerator)}) / {denominator} ({type(denominator)})"
                        )

                        if denominator == 0 or denominator is None or numerator is None:
                            logger.error(
                                f"Invalid interpolation values: numerator={numerator}, denominator={denominator}"
                            )
    ...
    def update_data_storage(self, new_data: Dict[str, List]):
        ...
        if self.temperatures:
            self.statistics_cache["current"] = self.calculate_temperature_trends(
                self.temperatures, self.dates
            )
    ...
    def _identify_extreme_events(self, data: Dict[str, List]) -> List[Dict[str, Any]]:
        ...
        for i, (temp, wind, precip, date) in enumerate(
            zip(temperatures, wind_speeds, precipitation, dates)
        ):
    ...
    def calculate_trend(self, values: List[float]) -> Dict[str, Any]:
        ...
        return {"direction": direction, "slope": slope, "correlation": correlation}
        ...
        if cache_key not in self.historical_data:
            if self.data_generator:
                ...
                self.historical_data[cache_key] = (
                    self.data_generator.generate_historical_comparison_data(timeframe)
                )
            else:
                ...
                if "temperatures" in historical_data:
                    historical_data["temperatures"] = [
                        t - 2 + (i % 3) for i, t in enumerate(historical_data["temperatures"])
                    ]
    ...
    def get_multi_city_data(self, cities: List[str], timeframe: str) -> Dict[str, Any]:
        ...
        if cache_key not in self.multi_city_data:
            if self.data_generator:
                ...
                self.multi_city_data[cache_key] = self.data_generator.generate_multi_city_data(
                    cities, timeframe
                )
            else:
