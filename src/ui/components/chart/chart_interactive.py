...
class ChartInteractiveMixin:
    ...
    def _create_export_controls(self):
        ...
        self.export_png_btn = ctk.CTkButton(
            self.export_frame,
            text="📸 PNG",
            command=self._export_high_res_png,
            **button_config
        )
        ...
        for metric, data in self.hover_data.items():
            if 'times' in data and 'values' in data:
                ...
                for i, (time, value) in enumerate(zip(times, values)):
                    ...
                    dx = abs(time_numeric - x) / (max(times).timestamp() - min(times).timestamp() if hasattr(times[0], 'timestamp') else len(times))
                    dy = abs(value - y) / (max(values) - min(values)) if max(values) != min(values) else 0
                ...
                with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                    ...
                    if self.hover_data:
                        ...
                        for i, time in enumerate(times):
                            row = [time.strftime('%Y-%m-%d %H:%M:%S') if hasattr(time, 'strftime') else str(time)]
