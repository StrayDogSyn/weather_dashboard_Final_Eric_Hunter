...
class ChartExportMixin:
    ...
    def export_chart(
        self, format_type: str = "png", filename: Optional[str] = None, high_quality: bool = True
    ):
        """Export chart to specified format with enhanced quality and glassmorphic styling."""
        try:
            if not hasattr(self, "fig"):
                raise ValueError("No chart available to export")

            # Prepare chart for export
            self._prepare_chart_for_export(high_quality)

            if format_type.lower() == "png":
                self._export_png(filename, high_quality)
            elif format_type.lower() == "pdf":
                self._export_pdf(filename, high_quality)
            ...
            self._restore_chart_after_export()

        except Exception as e:
            self.show_notification(f"Export failed: {str(e)}", "error")

    def _export_png(self, filename: Optional[str] = None, high_quality: bool = True):
        """Export chart as high-quality PNG with glassmorphic styling."""
        if not filename:
        ...
        # Configure enhanced export settings
        export_settings = self._get_png_export_settings()
        if high_quality:
            export_settings["dpi"] = 600
            export_settings["facecolor"] = "#0a0a0a"
            export_settings["metadata"] = {
                "Title": "Weather Dashboard Temperature Chart",
                "Author": "Weather Dashboard",
                "Subject": "Temperature Visualization",
                "Creator": "Weather Dashboard v1.0",
            }
    ...
    def _export_pdf(self, filename: Optional[str] = None, high_quality: bool = True):
        """Export chart as high-quality PDF with glassmorphic styling."""
        if not filename:
        ...
        with PdfPages(filename) as pdf:
            ...
            pdf.savefig(self.fig, **export_settings)

            # Add enhanced metadata
            metadata = self._get_export_metadata()
            if high_quality:
                metadata["Subject"] = "Temperature Visualization with Glassmorphic Design"
                metadata["Keywords"] = f"{metadata.get('Keywords', '')}, Glassmorphic"
    ...
    def _prepare_chart_for_export(self, high_quality=True):
        ...
        try:
            ...
            self._original_facecolor = self.fig.get_facecolor()
            ...
            print(f"Chart preparation error: {e}")

    def _restore_chart_after_export(self):
        """Restore chart to original state after export."""
        try:
            if not hasattr(self, "fig") or not hasattr(self, "ax"):
                return
            ...
            if hasattr(self, "_original_facecolor"):
                self.fig.set_facecolor(self._original_facecolor)
            ...
            print(f"Chart restoration error: {e}")

    def _export_svg(self, filename=None):
        """Export chart as scalable SVG format."""
        try:
            if not filename:
                filename = self._get_export_filename("svg")

            if not filename:
                return False

            # SVG export with enhanced settings
            self.fig.savefig(
                filename,
                format="svg",
                bbox_inches="tight",
    ...
    def _prepare_csv_data(self) -> str:
        ...
        for i, (date, temp) in enumerate(zip(self.dates, self.temperatures)):
            row = [date.strftime("%Y-%m-%d"), date.strftime("%H:%M:%S"), str(temp)]

            # Add additional data if available
            if (
                hasattr(self, "humidity_data")
                and self.humidity_data
                and i < len(self.humidity_data)
            ):
            ...
            if (
                hasattr(self, "pressure_data")
                and self.pressure_data
                and i < len(self.pressure_data)
            ):
    ...
    def _prepare_table_data(self) -> Dict[str, Any]:
        ...
        for i in range(0, len(self.dates), step):
            ...
            row = [self.dates[i].strftime("%m/%d %H:%M"), f"{self.temperatures[i]:.1f}"]
    ...
    def validate_export_data(self) -> bool:
        ...
        return (
            hasattr(self, "dates")
            and self.dates
            and hasattr(self, "temperatures")
            and self.temperatures
        )
