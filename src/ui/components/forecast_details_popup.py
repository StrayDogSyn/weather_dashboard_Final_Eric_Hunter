...
class ForecastDetailsPopup:
    ...
    def __init__(self, parent, forecast_data: Dict[str, Any], hourly_data: Optional[List] = None):
        ...
        self.popup.title(f"Forecast Details - {forecast_data.get('day', 'Day')} {forecast_data.get('date', '')}")
    ...
    def _create_overview(self, parent):
        ...
        # Temperature range
        temp_frame = ctk.CTkFrame(metrics_frame, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=8)
        ...
        # Precipitation
        precip_frame = ctk.CTkFrame(metrics_frame, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=8)
        ...
        precip_text = f"{int(precip_prob * 100)}%" if isinstance(precip_prob, float) else f"{precip_prob}%"
        ctk.CTkLabel(
            precip_frame,
            text=precip_text,
            font=("Consolas", 16, "bold"),
        ...
        # Humidity
        humidity_frame = ctk.CTkFrame(metrics_frame, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=8)
        ...
        # Wind
        wind_frame = ctk.CTkFrame(metrics_frame, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=8)
    ...
    def _create_hourly_section(self, parent):
        ...
        for hour_data in sample_hours:
            hour_frame = ctk.CTkFrame(hourly_container, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=6)
    ...
    def _create_details_section(self, parent):
        ...
        for i, (label, value) in enumerate(details):
            detail_frame = ctk.CTkFrame(details_container, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=6)
    ...
    def _create_accuracy_section(self, parent):
        ...
        for label, value, color in accuracy_info:
            info_frame = ctk.CTkFrame(accuracy_container, fg_color=DataTerminalTheme.BG_SECONDARY, corner_radius=6)
