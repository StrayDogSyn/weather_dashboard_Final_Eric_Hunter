...
class JournalManager(tk.Frame):
    ...
    def _create_list_panel(self) -> None:
        ...
        # List header
        list_header = tk.Frame(list_container, bg=self.theme.colors["glass_bg"], height=50)
        ...
        self.new_entry_btn = self._create_glass_button(
            list_header, "+ New Entry", self._create_new_entry, bg_color=self.theme.colors["accent"]
        )
        self.new_entry_btn.pack(side="right", pady=10)

        # Journal list
        self.journal_list = JournalList(
            list_container, journal_service=self.journal_service, theme=self.theme
        )
    ...
    def _create_editor_panel(self) -> None:
        ...
        # Editor header
        editor_header = tk.Frame(editor_container, bg=self.theme.colors["glass_bg"], height=50)
        ...
        self.save_btn = self._create_glass_button(
            action_frame, "💾 Save", self._save_entry, bg_color=self.theme.colors["success"]
        )
        self.save_btn.pack(side="left", padx=2)

        self.cancel_btn = self._create_glass_button(
            action_frame, "❌ Cancel", self._cancel_editing, bg_color=self.theme.colors["error"]
        )
        self.cancel_btn.pack(side="left", padx=2)

        # Journal editor
        self.journal_editor = JournalEditor(
            editor_container, journal_service=self.journal_service, theme=self.theme
        )
    ...
    def _create_separator(self) -> None:
        ...
        separator = tk.Frame(self, bg=self.theme.colors["border"], width=1)
        separator.grid(row=0, column=0, sticky="ns", padx=(0, 5))

    def _create_glass_button(
        self, parent: tk.Widget, text: str, command, bg_color: str = None
    ) -> tk.Button:
    ...
    def _on_entry_selected(self, entry: JournalEntry) -> None:
        ...
        self.editor_title.configure(
            text=f"✏️ Edit Entry - {entry.date_created.strftime('%Y-%m-%d %H:%M')}"
        )
        self._set_editor_state(True)
    ...
    def _cancel_editing(self) -> None:
        ...
        if self.is_editing and self.journal_editor.has_unsaved_changes():
            result = messagebox.askyesno(
                "Unsaved Changes", "You have unsaved changes. Are you sure you want to cancel?"
            )
            if not result:
                return
