...
class JournalList(tk.Frame):
    ...
    def _create_header(self, parent: tk.Widget) -> None:
        ...
        self.refresh_btn = self._create_glass_button(
            search_frame, "↻", self._refresh_entries, width=30
        )
    ...
    def _create_entry_list(self, parent: tk.Widget) -> None:
        ...
        canvas = tk.Canvas(list_frame, bg=self.theme.colors["glass_bg"], highlightthickness=0)
    ...
    def _create_footer(self, parent: tk.Widget) -> None:
        ...
        footer_frame = tk.Frame(parent, bg=self.theme.colors["glass_bg"], height=40)
    ...
    def _create_glass_button(
        self, parent: tk.Widget, text: str, command: Callable, width: int = 40, bg_color: str = None
    ) -> tk.Button:
    ...
    def _create_entry_card(self, entry: JournalEntry) -> tk.Frame:
        ...
        edit_btn = self._create_glass_button(
            action_frame,
            "Edit",
            lambda: self._select_entry(entry),
            width=40,
        ...
        for widget in [card, content_frame, header_frame, footer_frame, date_label, preview_label]:
    ...
    def _on_search_changed(self, *args) -> None:
        ...
        if not search_term:
            ...
            self.filtered_entries = [
                ...
                if (
                    search_term in entry.entry_content.lower()
                    or any(search_term in tag.lower() for tag in entry.tags)
                    or (entry.location and search_term in entry.location.lower())
                )
            ]
