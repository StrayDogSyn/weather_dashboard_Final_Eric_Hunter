...
class JournalEditor(tk.Frame):
    ...
    def _setup_ui(self) -> None:
        ...
        main_frame = tk.Frame(self, bg=self.theme.colors["glass_bg"], relief="flat", bd=0)
    ...
    def _create_header(self, parent: tk.Widget) -> None:
        ...
        header_frame = tk.Frame(parent, bg=self.theme.colors["glass_bg"], height=80)
        ...
        self.weather_frame = tk.Frame(header_frame, bg=self.theme.colors["glass_bg"])
    ...
    def _create_toolbar(self, parent: tk.Widget) -> None:
        ...
        toolbar_frame = tk.Frame(parent, bg=self.theme.colors["glass_bg"], height=50)
        ...
        self.bold_btn = self._create_glass_button(
            format_frame, "B", self._toggle_bold, font=(self.theme.fonts["body"][0], 12, "bold")
        )
        self.bold_btn.pack(side="left", padx=2)

        # Italic button
        self.italic_btn = self._create_glass_button(
            format_frame, "I", self._toggle_italic, font=(self.theme.fonts["body"][0], 12, "italic")
        )
        self.italic_btn.pack(side="left", padx=2)

        # Header button
        self.header_btn = self._create_glass_button(format_frame, "H", self._insert_header)
        self.header_btn.pack(side="left", padx=2)

        # List button
        self.list_btn = self._create_glass_button(format_frame, "•", self._insert_list)
        ...
        self.save_btn = self._create_glass_button(
            action_frame, "Save", self._save_entry, width=80, bg_color=self.theme.colors["accent"]
        )
        self.save_btn.pack(side="right", padx=5)

        # New button
        self.new_btn = self._create_glass_button(action_frame, "New", self._new_entry, width=60)
    ...
    def _create_footer(self, parent: tk.Widget) -> None:
        ...
        footer_frame = tk.Frame(parent, bg=self.theme.colors["glass_bg"], height=100)
    ...
    def _setup_markdown_highlighting(self) -> None:
        ...
        self.text_editor.tag_configure(
            "code", font=("Consolas", 10), background=self.theme.colors["code_bg"]
        )
    ...
    def _new_entry(self) -> None:
        ...
        if self.is_modified:
            result = messagebox.askyesnocancel(
                "Unsaved Changes", "You have unsaved changes. Save before creating new entry?"
            )
            if result is True:
                self._save_entry()
            elif result is None:
    ...
    def _on_entry_saved(self, auto_save: bool) -> None:
        ...
        if self.current_entry:
            self.title_label.configure(
                text=f"Entry - {self.current_entry.date_created.strftime('%Y-%m-%d %H:%M')}"
            )
