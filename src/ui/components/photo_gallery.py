...
class PhotoGalleryComponent(ttk.Frame):
    ...
    def _create_gallery_interface(self):
        ...
        # Remove photo button
        self.remove_button = ttk.Button(toolbar_frame, text="Remove Selected", 
                                       command=self._remove_selected_photo, state="disabled")
        self.remove_button.pack(side="left", padx=(5, 0))
        
        # View full size button
        self.view_button = ttk.Button(toolbar_frame, text="View Full Size", 
                                     command=self._view_full_size, state="disabled")
        ...
        v_scrollbar = ttk.Scrollbar(canvas_frame, orient="vertical", command=self.gallery_canvas.yview)
        h_scrollbar = ttk.Scrollbar(canvas_frame, orient="horizontal", command=self.gallery_canvas.xview)
        
        self.gallery_canvas.configure(yscrollcommand=v_scrollbar.set, xscrollcommand=h_scrollbar.set)
        ...
        self.canvas_window = self.gallery_canvas.create_window((0, 0), window=self.photos_frame, anchor="nw")
    ...
    def _process_new_photos(self, filenames: List[str]):
        ...
        try:
            ...
            if added_photos:
                ...
                messagebox.showinfo("Photos Added", f"Successfully added {len(added_photos)} photo(s).")
    ...
    def _refresh_gallery(self):
        ...
        for photo_path in self.current_entry.photos:
            ...
            if thumbnail:
                # Photo button
                photo_btn = tk.Button(thumb_frame, image=thumbnail, 
                ...
                placeholder = tk.Label(thumb_frame, text="Failed to\nload image", 
                                     width=15, height=8, bg="lightgray")
                placeholder.pack()
            
            # Photo name label
            name_label = ttk.Label(thumb_frame, text=Path(photo_path).name, 
