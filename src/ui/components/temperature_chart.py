...
class TemperatureChart(
    ...
    def _filter_data_by_range(self, data: Dict[str, Any], time_range: str) -> Dict[str, Any]:
        ...
        range_hours = {"6h": 6, "12h": 12, "24h": 24, "48h": 48, "7d": 168}  # 7 days * 24 hours
        ...
        if time_range in ["6h", "12h", "24h"]:
            ...
            if forecast_days and "hour" in forecast_days[0]:
                ...
                filtered_data["forecast"] = {
                    "forecastday": [{**forecast_days[0], "hour": filtered_hours}]
                }
        elif time_range == "48h":
            # Use first 2 days
                    ...
                    for hourly in forecast_obj.hourly_forecasts:
                        entry = {
                            ...
                            "main": {
                                ...
                                "feels_like": getattr(hourly, "feels_like", hourly.temperature)
                                + 273.15,
                                "humidity": getattr(hourly, "humidity", 60),
                                "pressure": getattr(hourly, "pressure", 1013),
                            },
                            "wind": {
                                "speed": getattr(hourly, "wind_speed", 0)
                                / 3.6  # Convert km/h to m/s
                            },
                            "weather": [{"main": getattr(hourly, "condition", "Clear")}],
                        }
