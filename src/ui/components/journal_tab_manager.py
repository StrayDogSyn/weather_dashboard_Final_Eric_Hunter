...
from ...models.journal import JournalEntry, Mood, JournalService, JournalRepository
from ..safe_widgets import SafeCTkFrame, SafeCTkLabel, SafeCTkButton, SafeCTkEntry, SafeCTkTextbox, SafeCTkComboBox
...
class JournalTabManager:
    ...
    def create_journal_tab(self) -> SafeCTkFrame:
        ...
        try:
            ...
            error_label = SafeCTkLabel(
                error_frame,
                text=f"Error creating journal tab: {str(e)}",
                text_color="red"
            )
    ...
    def _create_entry_form(self, entry: Optional[JournalEntry] = None):
        ...
        # Tags
        SafeCTkLabel(form_frame, text="Tags (comma-separated):").grid(row=5, column=0, sticky="w", pady=5)
        ...
        if entry and entry.weather_snapshot:
            ...
            SafeCTkLabel(
                weather_frame,
                text=f"Weather: {entry.weather_display}",
        ...
        try:
            if self.current_view == "edit" and self.current_entry and self.unsaved_changes:
                ...
                if self.auto_save_label:
                    self.auto_save_label.configure(text=f"Auto-saved at {datetime.now().strftime('%H:%M:%S')}")
    ...
    def _auto_fill_weather(self):
        ...
        if not self.weather_service or not self.location_entry:
            messagebox.showwarning("Warning", "Weather service not available or location not specified.")
            return
        ...
        try:
            ...
            if weather_data:
                ...
                self.current_entry.weather_snapshot = self.journal_service._create_weather_snapshot(weather_data)
                ...
                messagebox.showinfo("Success", f"Weather data filled: {self.current_entry.weather_display}")
    ...
    def _update_entry_from_form(self):
        ...
        # Update tags
        tags_text = self.tags_entry.get().strip()
        self.current_entry.tags = [tag.strip() for tag in tags_text.split(",") if tag.strip()] if tags_text else []
    ...
    def _get_filtered_entries(self) -> List[JournalEntry]:
        ...
        if self.filter_location:
            entries = [e for e in entries if e.location and self.filter_location.lower() in e.location.lower()]
    ...
    def _create_insights_view(self):
        ...
        # Basic statistics
        self._create_stats_section(scrollable_frame, "📈 Basic Statistics", {
            "Total Entries": stats.get('total_entries', 0),
            "Total Words": stats.get('total_words', 0),
            "Average Words per Entry": stats.get('average_words', 0),
            "Entries This Month": len(self.journal_service.get_entries_by_date_range(
        ...
        if mood_stats:
            mood_display = {f"{mood.emoji} {mood.value}": count for mood, count in mood_stats.items()}
        ...
        if weather_insights.get('most_common_moods_by_weather'):
            ...
            for weather, data in weather_insights['most_common_moods_by_weather'].items():
                weather_mood_display[f"{weather}"] = f"{data['emoji']} {data['mood']} ({data['count']} times)"
            
            self._create_stats_section(scrollable_frame, "🌤️ Weather & Mood Patterns", weather_mood_display, row)
            row += 1
        
        # Writing patterns
        writing_patterns = stats.get('writing_patterns', {})
        if writing_patterns.get('patterns'):
            patterns_display = {f"Pattern {i+1}": pattern for i, pattern in enumerate(writing_patterns['patterns'])}
            self._create_stats_section(scrollable_frame, "✍️ Writing Patterns", patterns_display, row)
            row += 1
        ...
        SafeCTkButton(
            scrollable_frame,
            text="☁️ Generate Word Cloud",
            command=self._show_word_cloud,
            width=200
