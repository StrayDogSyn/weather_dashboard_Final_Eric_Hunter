...
class LocationManager:
    ...
    def add_to_favorites(self, location: LocationData) -> bool:
        ...
        for fav in self.favorites:
            if abs(fav.lat - location.lat) < 0.01 and abs(fav.lon - location.lon) < 0.01:
    ...
    def add_to_recent(self, location: LocationData):
        ...
        self.recent_searches = [
            loc
            for loc in self.recent_searches
            if not (abs(loc.lat - location.lat) < 0.01 and abs(loc.lon - location.lon) < 0.01)
        ]
...
class LocationManagerUI(ctk.CTkFrame):
    ...
    def _create_favorites_section(self):
        ...
        self.favorites_frame = ctk.CTkScrollableFrame(
            favorites_frame, fg_color="transparent", height=150
        )
    ...
    def _create_recent_section(self):
        ...
        self.recent_frame = ctk.CTkScrollableFrame(recent_frame, fg_color="transparent", height=150)
    ...
    def _location_detected_success(self, location: LocationData):
        ...
        self.gps_status.configure(
            text=f"Current: {location.get_display_name()}", text_color=DataTerminalTheme.PRIMARY
        )
