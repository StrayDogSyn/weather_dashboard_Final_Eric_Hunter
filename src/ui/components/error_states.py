...
class APIErrorDisplay(ErrorStateComponent):
    ...
    def __init__(self, parent: tk.Widget, error_message: str, 
    ...
    def _get_user_friendly_message(self) -> str:
        ...
        if "timeout" in error_lower or "connection" in error_lower:
            ...
            return "We couldn't find weather data for this location. Please try a different location."
        else:
...
class LoadingState(ErrorStateComponent):
    ...
    def _start_animation(self):
        ...
        def spin_animation():
            ...
            while self.animation_running:
                try:
                    self.spinner_label.configure(text=spinner_chars[char_index % len(spinner_chars)])
                    char_index += 1
                    time.sleep(0.5)
                except tk.TclError:
                    break
...
class InputValidationError(ErrorStateComponent):
    ...
    def __init__(self, parent: tk.Widget, target_widget: tk.Widget, 
                 error_message: str, hint: str = None):
        super().__init__(parent)
...
class ErrorStateManager:
    ...
    def show_api_error(self, error_message: str, retry_callback: Callable = None) -> APIErrorDisplay:
        """Show API error."""
    ...
    def show_loading(self, message: str = "Loading...", show_progress: bool = False) -> LoadingState:
        """Show loading state."""
    ...
    def show_validation_error(self, target_widget: tk.Widget, error_message: str, 
                            hint: str = None) -> InputValidationError:
