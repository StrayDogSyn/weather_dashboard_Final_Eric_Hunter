...
class JournalCalendarComponent(ttk.Frame):
    ...
    def _create_calendar_interface(self):
        ...
        entries_scrollbar = ttk.Scrollbar(
            listbox_frame, orient="vertical", command=self.entries_listbox.yview
        )
        ...
        ttk.Button(action_frame, text="View Entry", command=self._view_selected_entry).pack(
            side="left"
        )
        ttk.Button(action_frame, text="New Entry", command=self._create_new_entry).pack(
            side="left", padx=(5, 0)
        )
    ...
    def _load_month_data(self):
        ...
        try:
            ...
            entries = self.journal_service.search_entries(
                date_range=(start_date, end_date), limit=1000  # Should be enough for a month
            )
