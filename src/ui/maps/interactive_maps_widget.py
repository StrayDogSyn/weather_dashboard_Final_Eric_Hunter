...
class InteractiveWeatherMapsWidget(ctk.CTkFrame):
    ...
    def _load_map_async(self):
        ...
        try:
            ...
            if self.show_stations_var.get():
                stations = asyncio.run(self.maps_service.get_weather_stations(
            ...
            if self.alert_vars.get("alerts", ctk.BooleanVar()).get():
                alerts = asyncio.run(self.maps_service.get_weather_alerts(
                    self.current_location[0], 
                    self.current_location[1]
                ))
            ...
            if not self._destroyed:
                self._safe_after(0, lambda: self._update_status(f"Error loading map: {e}"))
    ...
    def _search_location_async(self, location: str):
        ...
        try:
            ...
            if not self._destroyed:
                self._safe_after(0, lambda: self._update_status(f"Search error: {e}"))
    ...
    def _update_status(self, message: str):
        ...
        try:
            if not self._destroyed and hasattr(self, 'status_label') and self.status_label.winfo_exists():
    ...
    def _update_last_update_time(self):
        ...
        try:
            if not self._destroyed and hasattr(self, 'last_update_label') and self.last_update_label.winfo_exists():
