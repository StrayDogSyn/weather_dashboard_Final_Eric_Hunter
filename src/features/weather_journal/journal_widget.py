#!/usr/bin/env python3
"""
Weather Journal Widget - Main UI component for the weather journal feature

This module provides the main user interface for the weather journal,
integrating weather data with personal journaling capabilities.
...
from .export_system import ExportDialog
...
class WeatherJournalWidget(GlassFrame, LoggerMixin):
    ...
    - Tabbed interface (Write, Browse, Analytics, Search)
    - Rich text editor with Markdown support
    - Photo attachment system
    - Advanced analytics and data visualization
    - Professional export system
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.analytics_engine = AnalyticsEngine(database_manager)

        # UI components
        self.tabbed_interface: Optional[TabbedJournalInterface] = None
        ...
        self.logger.info("Weather Journal Widget (Phase 4) initialized")
    ...
    def _setup_ui(self):
        """Setup the journal user interface with tabbed design."""
    ...
    def _create_tabbed_interface(self):
        ...
        self.tabbed_interface = TabbedJournalInterface(
            self,
            controller=self.controller,
            analytics_engine=self.analytics_engine
        )
        self.tabbed_interface.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
        
        # Connect callbacks
        self.tabbed_interface.add_entry_selected_callback(self._on_entry_selected)
        self.tabbed_interface.add_export_callback(self._on_export_requested)
    ...
    def _export_entry(self):
        ...
        entry = self.controller.get_current_entry()
        if entry:
            dialog = ExportDialog(self, [entry])
            dialog.show()
    
    def _on_entry_selected(self, entry_id: int):
        """Handle entry selection from tabbed interface."""
        self._select_entry(entry_id)
    
    def _on_export_requested(self, entries: List[JournalEntry]):
        """Handle export request from tabbed interface."""
        dialog = ExportDialog(self, entries)
        dialog.show()
    ...
    def get_mood_analytics(self) -> Dict[str, Any]:
        ...
        return self.analytics_engine.get_mood_analytics()
    
    def get_analytics_visualization(self) -> AnalyticsVisualization:
        """Get analytics visualization component."""
        return AnalyticsVisualization(self.analytics_engine)
    
    def show_export_dialog(self, entries: Optional[List[JournalEntry]] = None):
        """Show export dialog for entries."""
        export_entries = entries or self.controller.get_all_entries()
        dialog = ExportDialog(self, export_entries)
        dialog.show()
