...
class LoadingManager:
    ...
    def load_critical(self, ui_setup_func: Callable, timeout: float = 2.0) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"✅ Critical loading completed in {elapsed:.2f}s")
    ...
    def load_deferred(self, tasks: List[Dict[str, Any]], max_timeout: float = 10.0) -> None:
        ...
        self.logger.info(f"🔄 Starting deferred loading of {len(tasks)} tasks...")
        ...
        def _run_deferred_tasks():
            ...
            self.logger.info(f"🏁 Deferred loading complete. Success: {len(completed_tasks)}, Failed: {len(failed_tasks)}")
            if failed_tasks:
                self.logger.warning(f"⚠️ Failed tasks: {', '.join(failed_tasks)}")
    ...
    def load_with_timeout(self, func: Callable, timeout: float = 5.0, fallback_result: Any = None) -> Any:
