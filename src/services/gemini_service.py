...
"""
        ...
        return ACTIVITY_PROMPT_TEMPLATE.format(
            ...
            category_preferences=", ".join(
                [cat.value for cat in user_preferences.favorite_categories]
            ),
    ...
    def _get_fallback_suggestions(
        self, weather_data: WeatherData, user_preferences: UserPreferences
    ) -> List[ActivitySuggestion]:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            response = await asyncio.to_thread(
                self.model.generate_content, "Test connection. Respond with 'OK'."
            )
