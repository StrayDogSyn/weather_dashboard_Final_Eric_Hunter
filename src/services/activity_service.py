...
"""
            ...
            try:
                ...
                if start_idx != -1 and end_idx != 0:
                    ...
                    for suggestion in suggestions[:5]:  # Limit to 5
                        if all(key in suggestion for key in ['title', 'category', 'description', 'time', 'items']):
                            # Add default icon if missing
                            if 'icon' not in suggestion:
                                suggestion['icon'] = '🎯' if suggestion['category'] == 'Outdoor' else '🏠'
                            validated_suggestions.append(suggestion)
                    
                    if validated_suggestions:
                        self.logger.info(f"Generated {len(validated_suggestions)} AI activity suggestions")
    ...
    def get_activity_by_category(self, weather_data: WeatherData, category: str) -> List[Dict[str, Any]]:
        """Get activities filtered by category (Indoor/Outdoor)."""
        all_activities = self.get_activity_suggestions(weather_data)
        return [activity for activity in all_activities if activity['category'].lower() == category.lower()]
