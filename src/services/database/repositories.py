...
class WeatherRepository(BaseRepository):
    ...
    async def get_latest_weather(self, location: str) -> Optional[WeatherHistory]:
        ...
        try:
            query = (
                ...
                .where(WeatherHistory.location.ilike(f"%{location}%"))
                .order_by(desc(WeatherHistory.timestamp))
                .limit(1)
            )
    ...
    async def get_weather_statistics(self, location: str, days: int = 30) -> Dict[str, Any]:
...
class PreferencesRepository(BaseRepository):
    ...
    async def save_preferences(
        self, preferences_data: Dict[str, Any], user_id: str = "default"
    ) -> UserPreferences:
    ...
    async def update_favorite_locations(
        self, locations: List[str], user_id: str = "default"
    ) -> None:
    ...
    async def add_recent_search(
        self, search: str, user_id: str = "default", max_searches: int = 10
    ) -> None:
        """Add a recent search.
...
class ActivityRepository(BaseRepository):
    ...
    async def log_activity(
        self, activity_data: Dict[str, Any], user_id: str = "default"
    ) -> ActivityLog:
    ...
    async def get_activity_history(
        ...
        try:
            ...
            query = select(ActivityLog).where(
                and_(ActivityLog.user_id == user_id, ActivityLog.selected_at >= start_date)
            )
    ...
    async def get_activity_statistics(
        ...
        try:
            ...
            ).where(and_(ActivityLog.user_id == user_id, ActivityLog.selected_at >= start_date))
            ...
            popular_query = (
                select(ActivityLog.activity_name, func.count(ActivityLog.id).label("count"))
                .where(and_(ActivityLog.user_id == user_id, ActivityLog.selected_at >= start_date))
                .group_by(ActivityLog.activity_name)
            ...
            return {
                ...
                "completed_activities": (
                    int(row.completed_activities) if row.completed_activities else 0
                ),
                "completion_rate": (
                    (row.completed_activities / row.total_activities * 100)
                    if row.total_activities and row.completed_activities
                    else 0
                ),
...
class JournalRepository(BaseRepository):
    ...
    async def create_entry(
        self, entry_data: Dict[str, Any], user_id: str = "default"
    ) -> JournalEntry:
    ...
    async def get_mood_trends(self, user_id: str = "default", days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            ).where(and_(JournalEntry.user_id == user_id, JournalEntry.date >= start_date))
            ...
            mood_query = (
                select(JournalEntry.mood, func.count(JournalEntry.id).label("count"))
                .where(and_(JournalEntry.user_id == user_id, JournalEntry.date >= start_date))
                .group_by(JournalEntry.mood)
            )

            mood_result = await self._session.execute(mood_query)
            mood_distribution = {row.mood: row.count for row in mood_result.fetchall() if row.mood}
