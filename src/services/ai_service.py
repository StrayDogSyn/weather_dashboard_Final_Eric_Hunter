...
class AIService:
        ...
        if GENAI_AVAILABLE:
            try:
                ...
                if api_key and api_key.strip():
                    ...
                    gemini_model_names = [
                        m.name for m in self.models if m.provider == ModelProvider.GEMINI
                    ]
    ...
    def _select_optimal_model(
        self, preferred_tier: Optional[ModelTier] = None, load_balance: bool = True
        ...
        available_models = [
            ...
            if m.name not in self.model_cooldowns or current_time > self.model_cooldowns[m.name]
        ]
            ...
            for model in available_models:
                ...
                try:
                    if (
                        ...
                        and model.avg_response_time
                        and model.cost_per_token > 0
                        and model.avg_response_time > 0
                    ):
    ...
    def _generate_with_openai(
        self, model_config: ModelConfig, prompt: str, system_prompt: str
    ) -> str:
    ...
    def generate_suggestions(
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(
                    f"✅ Generated response in {response_time:.2f}s using {model_config.name}"
                )
                ...
                if model_config:
                    ...
                    if any(
                        keyword in error_msg.lower() for keyword in ["rate limit", "429", "quota"]
                    ):
        ...
        # All attempts failed
        self.logger.error(
            f"❌ All AI models failed after {max_retries} attempts. Last error: {last_error}"
        )
    ...
    def generate_activity_suggestions(
        self, weather_data: Dict, user_preferences: Dict = None, prompt: str = None
    ) -> Dict:
        """
        ...
        try:
            ...
            response_text, model_used = self.generate_suggestions(
                prompt=prompt, system_prompt=system_prompt, preferred_tier=preferred_tier
            )
            ...
            try:
                ...
                if "activities" in suggestions and isinstance(suggestions["activities"], list):
                    self.logger.info(
                        f"✅ Generated {len(suggestions['activities'])} activities using {model_used}"
                    )
                    return suggestions
                else:
    ...
    def _create_activity_prompt(self, weather_data: Dict, user_preferences: Dict) -> str:
        ...
        print(
            f"🔍 Debug - user_preferences type: {type(user_preferences)}, value: {user_preferences}"
        )
