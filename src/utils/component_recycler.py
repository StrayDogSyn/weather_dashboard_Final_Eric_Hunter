...
class ComponentPool:
    ...
    def release(self, component: Any) -> bool:
        ...
        if component not in self._active_components:
            self.logger.warning(
                f"Attempting to release unknown component {type(component).__name__}"
            )
            return False
    ...
    def cleanup(self, force: bool = False) -> int:
        ...
        with self._lock:
            ...
            if cleaned_count > 0:
                self.logger.debug(
                    f"Cleaned up {cleaned_count} {self.component_type.__name__} components"
                )
...
class ComponentRecycler:
    ...
    def shutdown(self) -> None:
        ...
        self.logger.info(
            f"Component recycler shutdown complete. Cleared {total_cleared} components."
        )
