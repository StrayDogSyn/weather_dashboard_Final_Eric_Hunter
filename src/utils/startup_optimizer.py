...
class StartupOptimizer:
    ...
    def register_component(self, name: str, priority: ComponentPriority, 
                          loader_func: Callable[[], Any], 
    ...
    def _execute_progressive_loading(self, 
                                   on_component_loaded: Optional[Callable[[str, bool], None]] = None) -> None:
    ...
    def _resolve_dependencies(self, component_names: List[str]) -> List[str]:
        ...
        while remaining:
            ...
            for name in remaining:
                ...
                deps_satisfied = all(
                    dep in self.loaded_components or dep in resolved 
    ...
    def _load_priority_group(self, component_names: List[str],
                           on_component_loaded: Optional[Callable[[str, bool], None]] = None) -> None:
    ...
    def _load_sequential(self, component_names: List[str],
                        on_component_loaded: Optional[Callable[[str, bool], None]] = None) -> None:
    ...
    def _load_parallel(self, component_names: List[str],
                      on_component_loaded: Optional[Callable[[str, bool], None]] = None) -> None:
    ...
    def _load_component(self, name: str) -> LoadResult:
        ...
        for attempt in range(config.retry_count + 1):
            try:
                ...
                component_result = self._execute_with_timeout(
                    config.loader_func, 
                    config.timeout
                )
                ...
                last_error = e
                self.logger.warning(f"Failed to load component '{name}' (attempt {attempt + 1}): {e}")
        ...
        self.logger.error(f"✗ Component '{name}' failed to load after {config.retry_count + 1} attempts")
        return result
    ...
    def is_component_loaded(self, name: str) -> bool:
        ...
        return (name in self.loaded_components and 
                self.loaded_components[name].success)
    ...
    def get_loading_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'progress_percent': (loaded_count / total_components * 100) if total_components > 0 else 0,
            'components': {
                name: {
