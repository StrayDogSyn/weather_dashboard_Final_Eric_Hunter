...
import time
import logging
from typing import Callable, Dict, Any, Optional, Set, Awaitable
...
class LoadingManager:
    ...
    def load_critical(self, task: Callable, on_success: Callable = None, on_error: Callable = None, 
                     timeout: float = 30.0, task_name: str = "critical_task") -> str:
    ...
    def load_background(self, task: Callable, on_success: Callable = None, on_error: Callable = None,
                       timeout: float = 15.0, task_name: str = "background_task") -> str:
    ...
    def _execute_task(self, task_func: Callable, on_success: Callable = None, on_error: Callable = None,
                     timeout: float = 30.0, task_name: str = "task", priority: str = "normal") -> str:
        ...
        def task_wrapper():
            ...
            try:
                ...
                elapsed = time.time() - start_time
                self.logger.error(f"❌ Task {task_name} failed after {elapsed:.2f}s: {e}")
                if on_error:
                    try:
                        on_error(e)
                    except Exception as callback_error:
                        self.logger.error(f"Error callback failed for {task_name}: {callback_error}")
