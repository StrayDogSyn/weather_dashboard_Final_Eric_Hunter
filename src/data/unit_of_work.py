...
from .repositories.weather_repository import WeatherRepository, ForecastRepository
...
class UnitOfWork(IUnitOfWork):
    ...
    def register_operation(self, operation_type: str, repository: str, entity_id: str, details: Optional[Dict[str, Any]] = None):
...
class TransactionManager:
    ...
    def get_transaction_stats(self) -> Dict[str, Any]:
        ...
        successful = sum(1 for t in self._transaction_history if t.state == TransactionState.COMMITTED)
        failed = sum(1 for t in self._transaction_history if t.state == TransactionState.FAILED)
        rolled_back = sum(1 for t in self._transaction_history if t.state == TransactionState.ROLLED_BACK)
