...
class ActivityRepository(BaseRepository[ActivityRecommendation, str]):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_activity_type ON activity_recommendations(activity_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_activity_location ON activity_recommendations(location)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_activity_user ON activity_recommendations(user_id)")
    ...
    async def get_by_id(self, activity_id: str) -> Optional[ActivityRecommendation]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT activity_data FROM activity_recommendations WHERE id = ?",
                (activity_id,)
            )
            row = cursor.fetchone()
    ...
    async def get_all(self, limit: Optional[int] = None, offset: int = 0) -> List[ActivityRecommendation]:
    ...
    async def update(self, activity_id: str, entity: ActivityRecommendation) -> Optional[ActivityRecommendation]:
    ...
    async def delete(self, activity_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("DELETE FROM activity_recommendations WHERE id = ?", (activity_id,))
            conn.commit()
    ...
    async def exists(self, activity_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT 1 FROM activity_recommendations WHERE id = ?",
                (activity_id,)
            )
    ...
    async def get_recommendations_for_weather(self, temperature: float, wind_speed: float, precipitation: float, visibility: float) -> List[ActivityRecommendation]:
    ...
    async def get_by_activity_type(self, activity_type: ActivityType, limit: int = 10) -> List[ActivityRecommendation]:
    ...
    async def get_user_recommendations(self, user_id: str, limit: int = 20) -> List[ActivityRecommendation]:
    ...
    async def set_user_activity_preference(self, user_id: str, activity_type: ActivityType, preference_score: float) -> bool:
    ...
    async def log_activity_completion(self, user_id: str, activity_name: str, activity_type: ActivityType, weather_conditions: Dict[str, Any], user_rating: int) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                (user_id, activity_name, activity_type.value, json.dumps(weather_conditions), user_rating)
            )
            conn.commit()
            return True
    ...
    async def get_popular_activities(self, activity_type: Optional[ActivityType] = None, limit: int = 10) -> List[Tuple[str, int, float]]:
