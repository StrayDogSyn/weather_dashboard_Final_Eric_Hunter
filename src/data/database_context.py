...
class DatabaseContext(IDatabaseContext):
        ...
        with self._transaction_lock:
            ...
            try:
                ...
                for db_name, db_path in self.database_paths.items():
                    ...
                    self._transaction_connections_map[transaction_id].append(f"{transaction_id}_{conn_id}")
    ...
    async def commit_transaction(self) -> bool:
        ...
        with self._transaction_lock:
            ...
            try:
                ...
                await self._update_connection_states_after_transaction(transaction_id, ConnectionState.OPEN)
    ...
    async def _update_connection_states_after_transaction(self, transaction_id: str, new_state: ConnectionState):
    ...
    async def execute_query(self, database_name: str, query: str, params: tuple = ()) -> sqlite3.Cursor:
        """Execute query on specified database."""
        conn = await self.get_connection(database_name)
        return conn.execute(query, params)
    
    async def execute_many(self, database_name: str, query: str, params_list: List[tuple]) -> sqlite3.Cursor:
