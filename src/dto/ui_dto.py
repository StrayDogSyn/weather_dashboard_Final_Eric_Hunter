...
class FormattedValueDTO:
    ...
    @classmethod
    def temperature(cls, celsius: float, unit: str = "°C", precision: int = 1) -> "FormattedValueDTO":
    ...
    def wind_speed(cls, mps: float, unit: str = "m/s", precision: int = 1) -> "FormattedValueDTO":
        ...
        if unit == "km/h":
            ...
            return cls(
                value=kmh,
                unit=unit,
                formatted=f"{kmh:.{precision}f} {unit}",
                precision=precision
            )
        elif unit == "mph":
            mph = mps * 2.237
            return cls(
                value=mph,
                unit=unit,
                formatted=f"{mph:.{precision}f} {unit}",
                precision=precision
            )
        return cls(
            value=mps,
            unit=unit,
            formatted=f"{mps:.{precision}f} {unit}",
            precision=precision
        )
    ...
    def pressure(cls, hpa: float, unit: str = "hPa", precision: int = 0) -> "FormattedValueDTO":
        ...
        if unit == "inHg":
            ...
            return cls(
                value=inhg,
                unit=unit,
                formatted=f"{inhg:.{precision}f} {unit}",
                precision=precision
            )
        return cls(
            value=hpa,
            unit=unit,
            formatted=f"{hpa:.{precision}f} {unit}",
            precision=precision
        )
...
class ChartDataDTO:
    ...
    @classmethod
    def temperature_chart(cls, timestamps: List[str], temperatures: List[float], 
                         unit: str = "°C") -> "ChartDataDTO":
    ...
    @classmethod
    def precipitation_chart(cls, timestamps: List[str], precipitation: List[float]) -> "ChartDataDTO":
...
class DashboardLayoutDTO:
    ...
    def add_widget(self, widget_type: str, x: int, y: int, w: int, h: int, 
    ...
    def default_layout(cls) -> "DashboardLayoutDTO":
        ...
        layout.add_widget("current_weather", 0, 0, 6, 4, {
            "show_details": True,
            "display_mode": "detailed"
        })
        
        # Forecast widget
        layout.add_widget("forecast", 6, 0, 6, 4, {
            "forecast_type": "hourly",
            "hours": 12
        })
        
        # Alerts widget
        layout.add_widget("alerts", 0, 4, 12, 2, {
            "show_inactive": False
        })
        
        # Chart widget
        layout.add_widget("temperature_chart", 0, 6, 12, 4, {
            "chart_type": "line",
            "hours": 24
        })
...
class WeatherDashboardDTO:
    ...
    def to_display_dict(self, display_mode: DisplayMode = DisplayMode.DETAILED) -> Dict[str, Any]:
        ...
        if self.notifications:
            result["notifications"] = [notif.to_toast_dict() for notif in self.notifications if not notif.is_read]
